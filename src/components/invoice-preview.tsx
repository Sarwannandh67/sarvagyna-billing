    ...
    >
      ...
      <div className="flex flex-row justify-between items-start mb-10">
        <div className="mb-0">
          ...
          <h1 
            ...
            style={{ color: customColors?.primaryColor || 'black' }}
          >
            INVOICE
          </h1>
          <p 
            className="text-sm"
            style={{ color: customColors?.secondaryColor || 'var(--color-gold-dark)' }}
          >
        ...
        <div className="text-right">
          <p 
            ...
            style={{ color: customColors?.primaryColor || 'inherit' }}
          >
            {from.name || 'Your Company Name'}
          </p>
          <p 
            className="text-sm"
            style={{ color: customColors?.secondaryColor || 'var(--color-gold-darkest)' }}
          >
            {from.address || 'Your Company Address'}
          </p>
          {from.email && (
            <p 
              className="text-sm"
      ...
      <div className="grid grid-cols-2 gap-8 mb-10">
        ...
        <div className="text-right">
          <div className="mb-3">
            <p 
              className="text-sm font-semibold uppercase text-muted-foreground mb-1"
              style={{ color: customColors?.secondaryColor || 'inherit' }}
            >
              Issue Date
            </p>
      ...
      <div className="mb-10 overflow-hidden w-full">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr>
              <th 
                className="p-3 font-semibold text-sm border-b border-gray-200" 
                style={{ 
                  width: '50%', 
                  backgroundColor: customColors?.secondaryColor || '#f0f0f0',
                  color: customColors?.textColor || 'inherit'
                }}
              >
                Description
              </th>
              <th 
                className="p-3 font-semibold text-sm text-right border-b border-gray-200" 
                style={{ 
                  width: '15%', 
                  backgroundColor: customColors?.secondaryColor || '#f0f0f0',
                  color: customColors?.textColor || 'inherit'
          ...
          <tbody>
            {(items && items.length > 0 && items.some(item => item.description || (item.quantity && item.unitPrice) || (item.quantity > 0 && item.unitPrice > 0) )) ? ( 
                items.map((item) => (
                  <tr 
                    key={item.id} 
                    className="border-b border-[var(--color-gold-light)]"
                    style={{ 
                      borderBottomColor: customColors?.secondaryColor || 'var(--color-gold-light)' 
                    }}
                  >
      ...
      <div className="flex justify-end mb-10">
        <div className="w-full max-w-xs space-y-2">
          <div className="flex justify-between text-sm">
            <span 
              className="text-[var(--color-gold-darkest)]"
              style={{ color: customColors?.secondaryColor || 'var(--color-gold-darkest)' }}
            >
              Subtotal:
            </span>
      ...
      {notes && notes.trim() !== '' && (
        <div className="mb-6">
          ...
          <p 
            ...
            style={{ color: customColors?.textColor || 'var(--color-gold-darkest)' }}
          >
            {notes}
          </p>
        </div>
      ...
      {/* Footer */}
      <div 
        className="text-center text-sm pt-6 border-t mt-auto"
        style={{ 
          borderTopColor: customColors?.secondaryColor || 'var(--color-gold-light)',
          color: customColors?.secondaryColor || 'var(--color-gold-dark)'
        }}
      >
